...
public class Selection_Object_Icecream : MonoBehaviour {
	...
	public float Rotate_Speed; // creatinga  public float for rorate speed
	...
	void Gyroscope_Move(){ // creating a seperate function for all gyroscope movement
		if (Object_Can_Move) { // checking to see if our object can move setting is on
			...
			transform.Rotate (-Vector3.forward, Input.acceleration.x * Time.deltaTime * Rotate_Speed); // ROtation code
